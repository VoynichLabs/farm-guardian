...
**Last verified end-to-end:** 2026-04-14 18:35 ET (Claude Opus 4.6 — v2.26.2, `usb-cam` physically moved to MacBook Air; `mba-cam` ffmpeg pinned to FaceTime HD by name so the index shift from the USB camera plug-in didn't hijack its stream)
...
| `usb-cam` | Generic USB webcam (1920×1080), portable — plug it into whichever host | Any host running the `usb-cam-host` service. **Currently MacBook Air 2013** (moved from the Mac Mini at 2026-04-14 18:20 ET). | `192.168.0.50:8089` | `http://192.168.0.50:8089/photo.jpg` — HTTP snapshot pull, 1920×1080 JPEG via the `usb-cam-host` FastAPI service at `/Users/markb/.local/farm-services/usb-cam-host/` on the MBA (not under `~/Documents/` — Big Sur sandboxes that), venv uses `opencv-python-headless==4.8.1.78` for a prebuilt wheel, env var `OPENCV_AVFOUNDATION_SKIP_AUTH=1` so Camera TCC gets granted manually via System Settings rather than fought for from a worker thread | `source: snapshot`, `snapshot_method: http_url` via `HttpUrlSnapshotSource` (v2.26.0, 5 s cadence) | off | Brooder interior (heat-lamp lit) |
...
| `mba-cam` | Built-in FaceTime HD webcam (Apple VendorID `0x106B`, ProductID `0x1570`, 720p max — supports 30fps only, 15fps is not a valid capture rate) on the MacBook Air 2013 (MacBookAir6,2) | MacBook Air 2013 (macOS Big Sur 11.7.11) | `192.168.0.50` (DHCP) | `rtsp://192.168.0.50:8554/mba-cam` (TCP — published by `ffmpeg` via AVFoundation → MediaMTX v1.13.1). **ffmpeg is pinned to `-i "FaceTime HD Camera"` by name** (not by index) as of v2.26.2 — when the USB camera was plugged into the MBA, it took AVFoundation index 0 and pushed FaceTime HD to index 1, which would have silently swapped what `mba-cam` was streaming if the ffmpeg LaunchAgent ever restarted with `-i 0`. | `rtsp_url_override`, OpenCV `VideoCapture` | off | Coop (FaceTime HD angle, not brooder — the USB camera now owns the brooder angle) |
...
|---|---|---|---|---|
| **Mac Mini "Bubba"** | `192.168.0.71` (WiFi/en1, currently — see drift note) | macOS 26.3, 14-core M4 Pro, 64 GB | `guardian.py` (manual `nohup`, PID varies — all 5 camera consumers + YOLOv8 + the FastAPI dashboard on `:6530`); `tools.pipeline.orchestrator` daemon (manual `nohup`, VLM enrichment of archived frames); `usb-cam-host` LaunchAgent on `:8089` (`com.farmguardian.usb-cam-host`, v2.26.0 — serves `/photo.jpg` from the locally-plugged USB webcam; moves with the camera to whichever host it's plugged into next); `cloudflared` tunnel publishing `:6530` to `guardian.markbarney.net` (outbound, no port forward needed) | LM Studio on `:1234` (GLM-4.6v-Flash + others); dev loop for this repo and `farm-2026` |
...
| **MacBook Air 2013** | `192.168.0.50` (WiFi, DHCP) | macOS Big Sur 11.7.11 (hardware ceiling — no upgrade possible), Intel Core i5 Haswell 1.3 GHz, 8 GB, Python 3.8.9 from `/Library/Developer/CommandLineTools/` | `com.farmguardian.mediamtx` LaunchAgent on `:8554` (MediaMTX v1.13.1 — v1.16.3+ link against macOS 12 SDK and dyld-fail on Big Sur); `com.farmguardian.mba-cam` LaunchAgent (ffmpeg 8.1-tessus static, AVFoundation `-i "FaceTime HD Camera"` by name at 720p30 → re-rate `-r 15` → libx264 ultrafast → push to `rtsp://127.0.0.1:8554/mba-cam`); **`com.farmguardian.usb-cam-host` LaunchAgent** on `:8089` (v2.26.2 — serves the generic USB webcam now plugged into this MBA; runtime at `/Users/markb/.local/farm-services/usb-cam-host/` outside `~/Documents/` to avoid Big Sur's per-app-folder TCC sandbox on LaunchAgents; venv uses `opencv-python-headless==4.8.1.78` for a prebuilt Big Sur Intel Py3.8 wheel; env `OPENCV_AVFOUNDATION_SKIP_AUTH=1`) | Screensaver disabled (`idleTime=0`, `askForPassword=0`); `pmset sleep=0 disksleep=0 displaysleep=0 standby=0 powernap=0 hibernatemode=0 autorestart=1`. Lid must stay open (clamshell firmware sleep cannot be overridden on this 2013 hardware). |

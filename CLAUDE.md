...
- **Camera 1 (house-yard):** Reolink E1 Outdoor Pro — ONVIF, RTSP, 4K, PTZ, WiFi. IP `192.168.0.88`. Shows as **`FarmGuardian1`** in the Reolink app and in the frame's bottom-right OSD. **🔴 Boss hand-aimed this camera in the Reolink app on 06-Aug-2026 and it is exactly where he wants it: pan `1885` (94.2°), looking at Birdcatraz. Verified same day that nothing overrides it — camera-side PTZ guard is off with no home position saved, AI auto-tracking is off, all six on-camera cruise slots are empty and disabled, `ptz.patrol_enabled` and `sky_watch.enabled` are both false, no deterrent rule uses PTZ, and no scheduled job sends anything but `/snapshot`. The aim is saved as camera preset id 5 `boss-birdcatraz-aim` — that is the way back if it is ever knocked off. Do NOT move it, and do NOT `preset/goto` any id other than 5 (0–4 are stale April aims). Turning on patrol would destroy the aim AND force zoom to 0.**

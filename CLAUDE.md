...
- **Instagram posting to `@pawel_and_pawleen`** — **READ FIRST: [`docs/20-Apr-2026-ig-scheduled-posting-architecture.md`](docs/20-Apr-2026-ig-scheduled-posting-architecture.md) plus [`docs/SOCIAL_MEDIA_MAP.md`](docs/SOCIAL_MEDIA_MAP.md).** Those docs describe how Instagram posting works right now — LaunchAgent-based publishing, Discord reactions as the mixed-lane quality gate, zero CLI for Boss, the explicit S7 exception, and the disabled throwback/on-this-day paths. Earlier plan docs are still correct for account voice / hashtag rules / history but don't describe the live flow.
  - **Current architecture (updated 2026-05-03):** `discord-reaction-sync` scrapes `#farm-2026` reactions into `image_archive.discord_reactions`; `social-publisher` handles hourly reacted-gem stories only; archive/on-this-day fallback is DISABLED (`tools/social/config.json::archive_fallback_enabled=false`); `ig-daily-carousel` runs daily 18:00; the mixed `ig-daily-reel` runs daily 18:00 with Discord preview approval; `ig-s7-daily-reel` runs daily 21:00 as an explicit no-approval S7 time-lapse exception and then mentions Mark (`<@293569238386606080>`) in Discord. Per-cycle auto-posting is DEAD (`instagram.enabled=false`); do not re-enable.
...
- **Camera 2 (s7-cam):** Samsung Galaxy S7 phone running IP Webcam app (RTSP Camera Server). RTSP over WiFi (UDP). IP `192.168.0.249`, port 5554. Stream URL: `rtsp://192.168.0.249:5554/camera`. No auth required. Fixed camera, no PTZ. Uses `rtsp_url_override` — no ONVIF. Detection disabled. **Power & host: standalone USB wall brick in the coop (current state as of 2026-05-06).** Power-chain history: MBA-USB → standalone brick (2026-04-26) → GWTC-USB (2026-05-02, ADB-authorised so the watchdog could remotely restart IP Webcam) → standalone brick again (≤2026-05-06). Today there is no MBA tether, no GWTC tether, **no ADB host of any kind**. Phone reachable only over WiFi at `192.168.0.249:8080` (HTTP) / `:5554` (RTSP). When IP Webcam crashes/wedges, recovery is hands-on at the phone — there is no remote ADB escape hatch in the current configuration. **Orientation is PORTRAIT (fixed on phone, deliberate decision 2026-04-21, v2.35.2).** IP Webcam's `orientation=portrait` + `photo_rotation=90` settings drive an EXIF Orientation=6 tag on the JPEG; `capture.py:_apply_exif_rotation` bakes the rotation in at the capture boundary before `cv2.imdecode` (which ignores EXIF). Every downstream consumer sees 1080×1920 portrait pixels. Physical phone rotation does NOT change orientation — it's set via `http://192.168.0.249:8080/settings/orientation?set=...` (values: `portrait`, `landscape`, `upsidedown`, `upsidedown_portrait`). Portrait is the conscious choice because the s7-cam's primary content destination is IG stories + reels, which are native 9:16. Backend helper is adaptive (reads whatever EXIF says), so flipping back to landscape later only requires flipping the phone-side setting; the pipeline follows. **Settings re-assertion:** `com.farmguardian.s7-settings-watchdog` ticks every 10 min and re-curls `/settings/orientation?set=portrait` + `/settings/photo_rotation?set=90` (plus whitebalance + focusmode) so an in-app crash that drops sticky settings recovers within one cycle. **Known wedge mode:** if IP Webcam's HTTP server stalls (`/photoaf.jpg` returns 0 bytes), the four settings re-curls in the same tick will also fail (`wb=0 fm=0 or=0 pr=0` in the watchdog log) and the camera serves landscape until the app self-recovers — the 2026-05-06 incident captured in CHANGELOG. The deployed plist still carries an SSH-to-GWTC ADB-recovery branch from before the standalone-power switch; that branch is permanent dead code as of 2026-04-26 and should be ripped out next time the plist is touched.

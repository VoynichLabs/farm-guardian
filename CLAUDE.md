...
— **and as of v2.71.8 (04-Sep-2026) that verdict is CORROBORATED, so it is safe to act on
again.** ⚠️ It was NOT safe before that: on 03-Sep-2026 it declared a circuit trip and sent
Boss out to flip a breaker that was fine — `duo2` archived a frame at *exactly* the second it
was called down. The old `verdict = "circuit" if down else "pi-only"` let **one un-retried TCP
probe on any single device** flip it, and the failure was a fast refusal, not packet loss, so a
longer timeout would not have helped. A device is now counted as powered if **EITHER** it
archived a frame within 3x its median cadence **OR** its port answers (retried once); only the
absence of both is down. **If you ever doubt an alert, the one-line check is still
`image_archive`:** a frame from `house-yard` or `duo2` in the last minute means the circuit is
FINE and this is a Pi-only fault. See
`docs/04-Sep-2026-farm-pi5-sd-card-failure-and-rebuild.md` and
`docs/04-Sep-2026-watchdog-circuit-verdict-fix-plan.md`. It posts
...
- **`~/bubba-workspace/skills/farm-pi5-camera-host/SKILL.md`** — **READ THIS BEFORE TOUCHING THE BIRDCATRAZ PI OR ITS CAMERAS.** How to reach `farm-pi5`, the `by-id` identity model and why no fallback is permitted, systemd service management, adding a camera, the 30-second triage table, the one-source-frame proof that settles "are you processing the image?", the **IR-cut filter diagnosis** for a camera that is perfect at night and washed out by day, and the traps that have already cost time (`custom.toml` silently does nothing on this image — use `userconf.txt`; never build a `$6$` hash through a shell, and note Python 3.13 removed `crypt`, so verify the hash with `openssl passwd -6 -salt <salt>`; the log's firstboot `cmdline.txt` grep is STALE for 2026 trixie images, which use `resize` and will falsely report "bad image write"; `camera_host.py` needs `fastapi` + `uvicorn` as well as opencv/numpy, or it restart-loops on `ModuleNotFoundError`; after setting the hostname also fix `/etc/hosts` `127.0.1.1` or every `sudo` warns; a card-less Pi still answers TCP; orphaned `ffmpeg` probes hold cameras open).
...
| 5b | `jieli-dashcam` | **`farm-pi5`** (Raspberry Pi 5 at Birdcatraz), `http://farm-pi5.local:8091` — car dashcam in PC-camera mode, Jieli "USB PHY 2.0", 1280x720 wide-angle | off | ✅ **live** — replugged and re-enabled 04-Sep-2026 after the SD-card rebuild (1280x720, `camera_open:true`, 0 failures). Note it had been unplugged since `2026-09-02T22:14:57Z`, ~16h BEFORE the card failed — a **separate, earlier fault**; don't attribute that gap to the card. **Moved to the Pi 05-Aug-2026** (was MacBook Air `:8091`). Time-lapse material, never a gem. **Re-aimed frequently by Boss — never record what it points at, in config, docs, or code.** The old bus-power constraint is gone: it has a powered hub on the Pi |

...
- **Instagram posting to `@pawel_and_pawleen`** — full operational runbook at **`~/bubba-workspace/skills/farm-instagram-post/SKILL.md`**. Plan docs in this repo:
  - **`docs/19-Apr-2026-instagram-posting-plan.md`** — narrative/architecture plan (hashtag library, account-voice rules, 6-hr cadence, iPhone pickup, reels scope). Hashtag library went through three drafts — current version has creator-branded slop removed and all tags verified against best-hashtags.com / top-hashtags.com / displaypurposes.com.
  - **`docs/20-Apr-2026-ig-poster-implementation-plan.md`** — V2.0 code implementation plan for the photo lane: `tools/pipeline/ig_poster.py` + `hashtags.yml` + `git_helper.py` + `scripts/ig-post.py` CLI + DB migration + orchestrator hook. Phased 1–8. **Phases 1–7 shipped as of 2026-04-20** (see CHANGELOG v2.29.0). **Photo auto-posting is currently LIVE** in `tools/pipeline/config.json` (`instagram.enabled=true`, `auto_dry_run=false`). To disable, flip either to false + `launchctl kickstart -k gui/$(id -u)/com.farmguardian.pipeline`.
  - **`docs/20-Apr-2026-ig-next-phases-plan.md`** — follow-on spec: Carousels (§2.1, deferred), Stories (§2.5), Reels (§3).
  - **`docs/20-Apr-2026-ig-phase-2-3-stories-reels-plan.md`** — the Stories + Reels implementation plan (operationalizes the next-phases spec). **Stories + Reels shipped as of 2026-04-20** in CHANGELOG v2.31.0. Both ship gated off by default (`instagram.stories.enabled=false`, `instagram.reels.enabled=false`) while the photo lane stays live — staged rollout. Reel auto-orchestration isn't wired (v1 is manual CLI only); stories have a `_maybe_post_to_story` hook ready to flip.
  - CLI is now mode-dispatched: `scripts/ig-post.py --mode {photo,story,reel} ...`. Default mode is `photo` so every prior invocation (`--gem-id N --caption "..."`) still works unchanged. Story mode: `--mode story --gem-id N` (no caption — Graph API rejects captions on stories). Reel mode: `--mode reel --gem-ids N,N,N,N,N,N --caption "..."` (2–10 gems, stitched into a 9:16 MP4 via `tools/pipeline/reel_stitcher.py` then posted as `media_type=REELS`). `--dry-run` works on all three modes.
  Tokens live in macOS keychain, never expire. **Key architectural gotcha:** IG's media fetcher rejects `guardian.markbarney.net/api/v1/images/gems/{id}/image?size=1920` (fetcher requires URLs ending in `.jpg`/`.png`/`.mp4`). Working path: commit media to `farm-2026/public/photos/<subdir>/<name>.{jpg,mp4}`, push, feed `https://raw.githubusercontent.com/VoynichLabs/farm-2026/main/public/photos/<subdir>/<name>.{jpg,mp4}` to Graph API — live the instant the push lands, no Railway wait. First post at `https://www.instagram.com/p/DXVpa4Ek4Lb/` (brooder carousel); second at `https://www.instagram.com/p/DXVumJjExmr/` (chick-on-laptop — note its caption has a stale "watching for hawks" line from before Boss's correction; caption edit didn't propagate; do not re-attempt without asking). **Do NOT frame Farm Guardian as a security/predator system in captions** — predator detection as coded doesn't work, and even if it did, a predator on camera = a dead bird, not content. Stick to brooder/yorkie/flock/coop/yard-diary content. Hashtag library has a runtime `forbidden` list that rejects `#markbarney*`, `#builtwithai`, `#aiassistedfarm`, etc. — don't try to re-introduce them.

...
**S7 social exception (retimed 22-Jul-2026):** S7 daily time-lapse Reels run separately from the mixed Reel. `com.farmguardian.ig-s7-daily-reel` fires at **12:00 local** (was 21:00), selects sharp/safe `s7-cam` frames, posts without final approval, then sends a Discord notice mentioning Mark as `<@293569238386606080>`. Mark's Discord user ID is `293569238386606080`.
...
- **`docs/skills-s7-adb-operations.md`** — **CURRENTLY INAPPLICABLE as of 2026-05-06.** ADB-via-GWTC was the recovery path on the 2026-05-02 → 2026-05-06 GWTC-USB setup, but the phone is now back on a standalone USB wall brick in the coop with no host of any kind. Power-chain history: MBA-USB → standalone (2026-04-26) → GWTC-USB (2026-05-02) → standalone again (≤2026-05-06). Keep the runbook for if the phone moves back to GWTC-USB; in the meantime the on-phone failure-mode notes (IP Webcam on Configuration screen = server stopped, etc.) are still useful when walking to the phone. For "S7 not broadcasting" diagnosis in the current standalone configuration, use the watchdog log + `/status.json` probe path.
...
- **Instagram posting to `@pawel_and_pawleen`** — **READ FIRST: [`docs/20-Apr-2026-ig-scheduled-posting-architecture.md`](docs/20-Apr-2026-ig-scheduled-posting-architecture.md) plus [`docs/SOCIAL_MEDIA_MAP.md`](docs/SOCIAL_MEDIA_MAP.md).** Those docs describe how Instagram posting works right now — LaunchAgent-based publishing, Discord reactions as the mixed-lane quality gate, zero CLI for Boss, the explicit S7 exception, and the disabled throwback/on-this-day paths. Earlier plan docs are still correct for account voice / hashtag rules / history but don't describe the live flow.
  - **Current architecture (verified 2026-07-22 against the live plists):** `discord-reaction-sync` scrapes `#farm-2026` reactions into `image_archive.discord_reactions` every 30 min; `social-publisher` handles hourly reacted-gem stories only; archive/on-this-day fallback is DISABLED (`tools/social/config.json::archive_fallback_enabled=false`); per-cycle auto-posting is DEAD (`instagram.enabled=false`) — do not re-enable. Live daily schedule: **house-yard reel 09:00, s7 reel 12:00, carousel 12:30, duo2 reel 15:00, mixed `ig-daily-reel` 18:00, s7-backlog reel 09/13/17/20, insights-fetch 23:30, weekly digest Sun 20:00, chicken-daily-pick weekdays 09:30, nextdoor 18:30.** The mixed reel **auto-publishes** (`approval_required=False` in `daily_reel_runner.py`) — the old "Boss must react to approve" gate is gone. Full lane table: [`docs/SOCIAL_MEDIA_MAP.md`](docs/SOCIAL_MEDIA_MAP.md).
...
| Gateway laptop (GWTC) | `ssh -o StrictHostKeyChecking=no markb@192.168.0.69` (IP moved from `.68`; rediscover via /24 sweep on `:8554`) | yes — pinned `c.cmd`. Reachable 2026-07-22, but its camera lane is disabled |
| Larry's MSI laptop | `ssh -o StrictHostKeyChecking=no user@192.168.0.194` | box answers SSH, but its camera services are down (likely sitting at the Windows login screen — AtLogOn tasks need an interactive login). Verify before relying on it |
...
**Camera roster — SEVEN cameras, verified against both config files 2026-07-22.** Earlier versions of this section described only four; that was stale. `enabled` is Guardian's view (`config.json`); the pipeline (`tools/pipeline/config.json`) has its own flags and is noted where the two disagree.
...
| 3 | `usb-cam` | `usb-cam-host` on the **MacBook Air**, `Marks-MacBook-Air.local:8089` | off | live — **configs diverge, see below** |
| 4 | `gwtc` | Gateway laptop MediaMTX `rtsp://192.168.0.69:8554/gwtc` | off | **disabled in both configs** |
| 5 | `mba-cam` | MacBook Air `192.168.0.50:8089` | off | disabled in Guardian, **enabled in the pipeline** |
| 6 | `dominator-cam` | `192.168.0.194:8089` | off | enabled |
| 7 | `duo2` | Reolink Duo 2 WiFi, `rtsp://…@192.168.0.155:554` | **ON** | live |

⚠️ **Known config drift (not yet fixed):** `usb-cam` points at the MacBook Air in `config.json` but at `192.168.0.194:8090` in `tools/pipeline/config.json`; `mba-cam` is `enabled:false` for Guardian but `enabled:true` for the pipeline at `192.168.0.50:8089`. Reconcile before trusting either file about which host serves which camera.

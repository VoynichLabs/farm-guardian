...
| 3 | `usb-webcam-1080p` | `usb-cam-host` on the **MacBook Air**, `192.168.0.50:8090` (moved off GWTC 05-Aug-2026) | off | **⚠️ INTERMITTENT, currently 503** — on the USB bus (serial `240725172848`, powered) but its **video interface is absent from AVFoundation**, so there is nothing to serve. Needs a **physical replug**; a service restart is not a power cycle and does not clear it. The service is loaded and will pick it up automatically when it returns. **Not dead** — an earlier "dead camera" call here was an overclaim |

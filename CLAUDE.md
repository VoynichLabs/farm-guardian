...
- **GWTC reboots leave ffmpeg in a dshow zombie state — but a watchdog auto-recovers it.** After any GWTC reboot, the Shawl + ffmpeg + mediamtx services all report `Running`, ffmpeg has a live PID, port 8554 is open — but the `nestbox` RTSP path 404s because ffmpeg is wedged on the dshow camera open and never registers as a publisher. Neither Shawl's `--restart` policy nor the `:loop` retry in `start-camera.bat` triggers (the wedged ffmpeg never exits). **The `farmcam-watchdog` Windows service (deployed 13-Apr-2026) detects this within ~90s and kills the wedged ffmpeg PID; Shawl then respawns it cleanly. You should not need to intervene.** If the watchdog itself is broken (`sc query farmcam-watchdog` not `RUNNING`), fall back to manual recovery: `ssh markb@192.168.0.68 'tasklist | findstr ffmpeg'` then `taskkill /F /PID <pid>`. Watchdog code lives at `deploy/gwtc/farm-watchdog.ps1` with install recipe at `deploy/gwtc/install-watchdog.md`. Full failure-mode writeup with both the watchdog design and the manual fallback: `docs/13-Apr-2026-gwtc-laptop-troubleshooting-incident.md` "Addendum -- Post-Reboot dshow Zombie Pattern" section.

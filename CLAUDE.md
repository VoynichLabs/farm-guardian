...
**Camera roster — SEVEN cameras, verified against both config files 2026-07-23.** Earlier versions of this section described only four; that was stale. `enabled` is Guardian's view (`config.json`); the pipeline (`tools/pipeline/config.json`) has its own flags and is noted where the two disagree.
...
- **Camera 4 (gwtc):** Gateway laptop (Windows 11) built-in webcam, **currently `enabled: false` in both config files** — the lane is off, not merely unreachable.
  - **⚠️ State verified 23-Jul-2026: the laptop is FINE, the camera is DEAD.** GWTC is fully reachable at `192.168.0.69` (SSH 22 + MediaMTX 8554 both open), up since 20-Jul, and all three services (`farmcam`, `farmcam-watchdog`, `mediamtx`) report Running. But `Get-PnpDevice` shows **`Hy-HD-Camera` with `Present: False`** — the webcam is not on the device bus at all. `ffmpeg -list_devices` sees only an "OBS Virtual Camera" (OBS has been installed on the box at some point). So `start-camera.bat`, which opens `video="Hy-HD-Camera"`, exits instantly and Shawl respawns it forever.
  ...
  - **Screen (23-Jul-2026):** the panel read as black and Boss couldn't tell from the coop whether the machine was even running. It was: power timeouts were already `never`, but **brightness was pinned below visibility** (a leftover of the "birdproofing" done when the laptop sat in the nesting box — see `C:\farm-services\undo-birdproof.bat`). Fixed by `deploy/gwtc/screen-on.ps1`, deployed to `C:\farm-services\screen-on.ps1`: forces the monitor powered on, nudges the input stack, sets brightness to 90, and re-asserts the never-sleep timeouts. It runs from the `farmcam-screen-on` scheduled task on **two** triggers — at logon (covers reboots) and hourly (covers anything blanking it mid-run). Re-register with `deploy/gwtc/register-screen-task.ps1`.

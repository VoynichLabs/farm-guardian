...
- **GWTC reboots leave ffmpeg in a dshow zombie state.** After any GWTC reboot, the Shawl + ffmpeg + mediamtx services all report `Running`, ffmpeg has a live PID, port 8554 is open — but the `nestbox` RTSP path 404s because ffmpeg is wedged on the dshow camera open and never registers as a publisher. Neither Shawl's `--restart` policy nor the `:loop` retry in `start-camera.bat` triggers (the wedged ffmpeg never exits). **Fix: `ssh markb@192.168.0.68 'tasklist | findstr ffmpeg'` to find the PID, then `taskkill /F /PID <pid>` — Shawl respawns a healthy ffmpeg within ~3s and Guardian auto-reconnects within its 60s back-off.** Full failure-mode writeup, why the obvious fixes don't work, and the verified-on-2026-04-13 recipe: `docs/13-Apr-2026-gwtc-laptop-troubleshooting-incident.md` "Addendum — Post-Reboot dshow Zombie Pattern" section.

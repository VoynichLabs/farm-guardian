...
- **Instagram posting to `@pawel_and_pawleen`** — **READ FIRST: [`docs/20-Apr-2026-ig-scheduled-posting-architecture.md`](docs/20-Apr-2026-ig-scheduled-posting-architecture.md).** That doc is the canonical description of how Instagram posting works right now — four LaunchAgents, Discord reactions as the quality gate, zero CLI for Boss. Supersedes every earlier plan doc for architecture; the earlier docs are still correct for account voice / hashtag rules / history but don't describe the live flow.
  - **Current architecture (v2.36.6, 2026-04-23):** Four LaunchAgents: `discord-reaction-sync` (every 30 min — scrapes `#farm-2026` reactions and writes counts to `image_archive.discord_reactions`), `ig-2hr-story` (every 2h, **EVERY unposted reacted gem FIFO, capped at 25/tick for IG's 24h publish quota** — the earlier "best in window" behaviour dropped gems and was replaced 2026-04-23 per Boss directive "anything reacted is worthy"), `ig-daily-carousel` (daily 18:00 local, today's reacted strong+sharp gems), `ig-weekly-reel` (Sundays 19:00, past week's reacted strong+sharp gems stitched via ffmpeg). Per-cycle auto-posting is DEAD (`instagram.enabled=false`); do not re-enable.
  - **Zero-loss backstop:** `select_all_unposted_story_gems` in `tools/pipeline/ig_selection.py` has NO time window. If a gem got a Discord reaction but wasn't published (agent down, API error, whatever), the next 2h tick picks it up. That's the whole point — Boss's reaction is a commitment to publish. Large backlogs (e.g. 315+ from pre-fix drops) drain at 25/tick.
  - **IG publish quota — SHARED across both pipelines, 25 per rolling 24h.** Instagram's Graph API hard-caps Business accounts at 25 `media` publishes per rolling 24-hour window, and this counts BOTH the gem lane (`ig-2hr-story` + `ig-daily-carousel` + `ig-weekly-reel`) AND the on-this-day archive lane. When you hit the cap, Graph 403s every subsequent container-status poll until the oldest publishes age past 24h. Both the gem-lane script (`scripts/ig-2hr-story.py`) and the archive-lane cycle (`tools.on_this_day.post_daily.run_auto_story_cycle`) detect the 403 and stop the batch cleanly so the next tick resumes when a slot frees. **Do not "fix" the 403 by cranking up timeouts or retrying with new tokens — it's a hard quota, not an auth problem.** If Boss wants the pipelines to coordinate fairly (e.g. "gem lane always gets half the quota"), implement a shared 24h-publish ledger; the current behaviour is first-come-first-served, which means the more frequent archive-lane cadence (90 min vs 120 min) tends to win more slots on quiet gem days.
  - **Quality gate:** Human reactions on Discord `#farm-2026` are the ONLY filter. VLM `share_worth=strong, image_quality=sharp` is not sufficient by itself — it tagged a heat-lamp-orange-cast clipped frame as strong+sharp. Cross-reference from Discord message back to image_archive row is by `(camera_id, ts ±60s)` — NOT sha256 (Discord CDN re-encodes). Larry/Bubba/Egon are other Claude instances; their reactions don't count.
  ...
  - **Hard architectural gotcha:** IG's media fetcher rejects `guardian.markbarney.net/api/v1/images/gems/{id}/image?size=1920` (needs URL ending in `.jpg`/`.png`/`.mp4`). Working path is still commit to `farm-2026/public/photos/<subdir>/<name>.{jpg,mp4}` + push + feed the `raw.githubusercontent.com` URL to Graph API. Live the instant the push lands.
...
- **On-this-day archive lane — historical iPhone photos (FB Page + IG Stories, 90-min cadence)** — separate pipeline from the gem lane above. Source: the Qwen-catalogued 21k+ iPhone archive at `~/bubba-workspace/projects/photos-curation/photo-catalog/master-catalog.csv`. Selector: `tools/on_this_day/selector.py` picks today's calendar matches from 2022/2024/2025 (2023 skipped per Boss), falls back to back-catalog when today's pool is exhausted. Content filter is automated (farm/pet keywords in; hawks/predators/receipts/screenshots/etc. rejected); NO reaction gate — this is archival content, not live. Orchestrator: `com.farmguardian.on-this-day` fires `scripts/on-this-day-stories.py` → `tools.on_this_day.post_daily.run_auto_story_cycle` every 90 min, publishing ONE photo per tick to both FB + IG as 9:16 stories. State: `data/on-this-day/posted.json` (UUIDs never reposted) + `data/on-this-day/export-failures.json` (cloud-only photos blacklisted for the day, retry tomorrow) + `data/on-this-day/auto-story-YYYY-MM-DD.ndjson` (per-tick audit). Full readme: [`tools/on_this_day/README.md`](tools/on_this_day/README.md). Do NOT conflate with the gem lane — different source, different quality gate, different state files, different cadence.

- **Reciprocate harvester — list who engaged with the FB Page (4h cadence, Discord summary)** — `tools/on_this_day/reciprocate.py` fired by `com.farmguardian.reciprocate` every 4h. Hits Graph API for the last 2 days of Page posts + Stories, aggregates reactors/commenters, writes `data/on-this-day/engagers-YYYY-MM-DD.{json,txt}`, posts a top-15 click-list to Discord channel `1476787165638951026` (via the Bubba bot token in `~/.openclaw/openclaw.json`). **NOT `#farm-2026`** — that's the reaction-quality-gate for the gem lane and any pollution breaks the signal. FB Graph doesn't expose Page-follows-user, so reciprocation is a manual click-through; the tool surfaces the worklist.

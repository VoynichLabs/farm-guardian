...
**2026-05-02 social exception:** S7 daily time-lapse Reels run separately from the mixed reaction-gated Reel. `com.farmguardian.ig-s7-daily-reel` fires at 21:00 local, selects sharp/safe `s7-cam` frames, posts without final approval, then sends a Discord notice mentioning Mark as `<@293569238386606080>`. Mark's Discord user ID is `293569238386606080`.
...
- **Instagram posting to `@pawel_and_pawleen`** — **READ FIRST: [`docs/20-Apr-2026-ig-scheduled-posting-architecture.md`](docs/20-Apr-2026-ig-scheduled-posting-architecture.md).** That doc is the canonical description of how Instagram posting works right now — four LaunchAgents, Discord reactions as the quality gate, zero CLI for Boss. Supersedes every earlier plan doc for architecture; the earlier docs are still correct for account voice / hashtag rules / history but don't describe the live flow.
  - **Current architecture (updated 2026-05-03):** `discord-reaction-sync` scrapes `#farm-2026` reactions into `image_archive.discord_reactions`; `social-publisher` handles hourly reacted-gem stories + archive fallback; `ig-daily-carousel` runs daily 18:00; the mixed `ig-daily-reel` runs daily 18:00 with Discord preview approval; `ig-s7-daily-reel` runs daily 21:00 as an explicit no-approval S7 time-lapse exception and then mentions Mark (`<@293569238386606080>`) in Discord. Per-cycle auto-posting is DEAD (`instagram.enabled=false`); do not re-enable.

...
def _todays_observations(db_path: Path, log: logging.Logger, hours: int = 24) -> str:
    ...
    # The any_special_chick column holds the VLM's `standout_bird` since
    # v2.74.0 (21-Sep-2026): a frame where a bird stood out, not a "chick".
    if sum(1 for _a, _bc, _lt, sc in rows if sc):
        facts.append("a few birds really stood out")

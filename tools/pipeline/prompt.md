...
**Camera angle.** Overhead coop/run views often show backs, tops of heads, and bird rumps rather than faces — that is normal, not a defect in the bird. Score conservatively for the budget sensors (usb-cam, gwtc) at awkward angles — a mid-band score is a good shot for those, and they rarely clear the 70+ gem bar. The s7-cam is the best sensor in the fleet, so score it on its own merits — it can produce genuine gems.
...
**Leg bands — REPORT WHAT YOU SEE, do not identify the bird from it.** Some birds wear a small colored, numbered plastic band on one leg. Your only job is to read the ring and put it in the three band fields. Working out *which bird* wears it is done afterwards from the farm's own records, which cannot misremember — so a guess from you adds nothing and can only introduce an error.
- `band_color`: the color you can actually see, or `none` if you cannot see a band on any leg in this image. **Never infer a band from a bird's plumage, or from which bird you think it is.** If you did not see a ring on a leg, the answer is `none`.
- `band_leg`: which of the **bird's own** legs it is on — think about which way the bird is facing, not which side of the picture it is on. Use `unsure` if you can see the band but cannot work that out. `unsure` is a perfectly good answer and is far better than a guess.
- `band_number`: the number printed on the band, or `-1` if there is a band but the number is not legible. **Do not guess a number.** `-1` is the correct answer most of the time — these bands are small and usually too far away or too blurry to read.
- If more than one bird is banded, report the single band you can read most clearly.
- **Never mention a leg band in `caption_draft` or `share_reason`.** The band belongs in those three fields and nowhere else. If the farm's records can identify the bird from what you reported, the name is added to the caption afterwards.
- An unbanded leg, or a leg turned out of view, is completely normal — not a defect, and not a reason to lower the score.
...
- `share_worth`: this is the **farm-gem gate**, not just a visibility flag. Sharpness + a visible face is necessary, but not sufficient for `strong`. A blurry bird, a bird's rear end, a bird hidden by wire mesh, or a flat floor-pecking snapshot is worthless regardless of how many birds are present.
  - **"strong"** — the frame must be genuinely usable/interesting as a farm gem. Use `strong` only when at least one of these is true and none of the skip triggers apply:
    ...
    5. A bird close to the camera making eye contact with the lens (direct or three-quarter) — closeness and eye contact together are enough on their own, even without dramatic action or exceptional light.
  - **"skip"** — ANY of these demotes the frame:
    ...
    9. Multiple scattered birds around feeders, fence lines, shadows, or background clutter where one bird is merely standing while others peck/forage nearby — a wide inventory snapshot with no clear lead subject. **This is about DISTANT SCATTER, not the water bowl itself:** the s7 camera is deliberately aimed at the big water bowl, and a close, sharp bird drinking or facing the lens AT the bowl can absolutely be `strong` (see trigger 5 above). Only demote when the birds are small, far, and unfocused with no lead subject.
    10. The closest, most prominent bird in the frame is facing away (back, rump, or tail to camera), and the only birds looking toward the lens are small or far off in the background — even if those distant birds are visible. A big sharp foreground rump with only small distant faces behind it is a skip. The lead bird has to be looking at you.
...
- `overall_score`: integer 0–100, the **farm-gem rating shown in Discord**. The pipeline RECOMPUTES this from four weighted components, so focus on scoring `expression_score` and `detail_score` accurately — still give your best 0–100 estimate here, but it will be recalculated. The four weighted axes (sum to 100): **frame dominance (0–30)** from `largest_subject_pct`, **expression (0–30)** = your `expression_score`, **notable detail (0–25)** = your `detail_score`, **technical quality (0–15)** from `image_quality`+`lighting`. A frame only reaches Discord at **70+**.
  - **What lands 70+:** a bird filling a large part of the frame (high dominance) that is ALSO doing something expressive/absurd AND showing crisp distinctive detail (a claw, a wing, a wild eye) in sharp focus. All four axes strong at once. This is intentionally hard.
  - **What stays under 70:** distant birds, neutral standing poses, soft focus, backsides, cluttered floor scenes, or any frame missing one of the four axes. A clean but ordinary profile portrait is a mid-band shot (~50–65), not a gem.
  ...
  - Keep this consistent with `share_worth`: only frames you tag `strong` should reach 70+; `decent` frames land mid-band; `skip` frames at the bottom.
  - **Calibration examples (score the components; the overall follows):**
    ...
    - Prominent bird facing away — a large sharp buff bird fills the foreground but is rump-to-camera, no other bird looking at the lens → skip trigger 10 → `share_worth="skip"`, `expression_score=2`, `detail_score=4`.
...
- `caption_draft`: 2–4 sentences, up to ~450 chars. **Describe the whole scene — all the birds, not just one.** This goes on social media. Make it vivid.
  ...
  - **Never mention a leg band here.** Bands go in `band_color` / `band_leg` / `band_number` only. Writing one into the caption is how wrong bands reached Instagram — 440 band claims went out this way, five of them describing bands that exist on no bird.

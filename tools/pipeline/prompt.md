...
**Leg bands — the most reliable ID when you can read one.** Many of the named birds above now wear a small colored, numbered plastic band on ONE leg — and **which leg (left or right) is part of the band's identity**, so it matters which leg you see it on. A clearly legible band is the single most reliable way to identify a bird — more reliable than plumage, since several of these birds are near-identical siblings. Use it by these rules, exactly:
- **Read all three — the color, the number, AND which leg it's on (left or right) — and match all three** against the list above. Band colors look alike when the band is small or dimly lit (red vs. orange vs. pink; green vs. blue), and the same color-and-number can belong to a different bird on the other leg, so the leg is part of the ID. Match on color + number + leg; never match on color alone.
...
- `caption_draft`: 2–4 sentences, up to ~450 chars. **Describe the whole scene — all the birds, not just one.** This goes on social media. Make it vivid.
  ...
  - **Leg band only when it's clearly legible.** If a bird wears a colored, numbered leg band that you can genuinely read in this frame, name it — including which leg: "green band #2 on the left leg," "purple band #12 on the right leg." This only applies when the band is actually visible; never invent one or state a color, number, or leg you cannot read.

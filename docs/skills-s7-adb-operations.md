...
> **STATUS as of 2026-08-01 (CURRENT) — ADB IS DEAD ON THIS PHONE, PERMANENTLY.** The S7 got wet and its **micro-USB port no longer works for power OR data**. Confirmed 2026-08-01 with a known-good DATA cable plugged DIRECTLY into the Mac mini: the phone does not enumerate on the USB bus at all — no Samsung vendor ID (1256 / 0x04E8) anywhere in `ioreg -rc IOUSBHostDevice`. It now charges on a **Qi wireless pad**, which is the only working charging path.
>
> **Everything below is historical.** It cannot be revived by re-tethering the phone, because no USB host can ever attach to it again. adb-over-USB needs a working port; adb-over-network is refused (5555 closed) because Android 8.0.0 predates wireless-debugging pairing and `adb tcpip` would itself require one working USB session. Do not plan recovery, app cleanup, or settings sweeps around ADB on this phone — walk to it instead. Charge-path diagnostic: `tools/s7-charge-diagnose.sh`.
>
> In the current configuration: settings re-assertion happens via HTTP only (`com.farmguardian.s7-settings-watchdog` → `curl http://192.168.0.249:8080/settings/...?set=...`); IP Webcam crash recovery is hands-on at the phone (walk to it, swipe-kill the app or back-arrow to Configuration → Start server). Power-chain history: MBA-USB → standalone (2026-04-26) → GWTC-USB (2026-05-02, the snapshot below) → standalone again (≤2026-05-06) → Qi pad (2026-08-01).
>

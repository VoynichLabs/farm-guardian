...
> **IP RESERVATION — DONE 2026-06-07.** GWTC's DHCP IP had been drifting (router lease was only 120 min); on 2026-06-07 it moved `.68` → `.69`, which silently broke *both* feeds from the Mini (both configs hardcode the IP). **Fixed permanently:** added a DHCP Address Reservation on the Archer AX55 (Advanced → Network → DHCP Server → Address Reservation) binding MAC `F0:35:75:81:2C:45` → `192.168.0.69`. GWTC now keeps `.69` across reboots; `config.json` + `tools/pipeline/config.json` point there. If the router is ever factory-reset, re-add this reservation.

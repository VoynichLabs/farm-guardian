...
**Verified live, by eye rather than by number** — resolution can no longer identify these cameras, so each port was pulled and the image looked at: `:8090` shows the coop run, `:8091` shows the yard with the truck and the house. Guardian serves both (`/api/cameras/<name>/frame` → HTTP 200, 327 KB and 494 KB), the pipeline is writing rows under the new ids, and the four touched pipeline self-tests pass. `macbook-air-facetime` was 503 at the time of writing because the built-in camera had vanished from macOS entirely — **and the "lid is shut" explanation first published here was wrong**: `ioreg -r -k AppleClamshellState` returned `No`, lid open, nobody had touched the machine. Re-seating the external USB hub restored it (the built-in is an internal USB device on the same controller, so the re-seat re-enumerates it too), and all three services then re-identified their cameras correctly through the reshuffle — the first real-world test of the new resolver, verified by looking at the frames: fence, coop run, yard. Trigger not isolated between an ffmpeg killed mid-open on that camera and the bus load of a third camera.
...
- **Corrected a false docstring that had stood for months.** `mark_gems_used_in_backlog_reel` claimed the marker also removed a gem from the Story queue — "so the same gem never gets posted as a story." It never did: `select_all_unposted_story_gems` filters on `ig_story_id IS NULL` and `story-permanent-skip:%` only and has never looked at this marker. Measured against the live archive: of 1,746 marked gems, **1,569 were posted as Stories as well.** Corrected in the docstring, `SOCIAL_MEDIA_MAP.md`, the plan doc and the runner comment, with an explicit "do not add the exclusion now" note — doing so would silently cut ~16 gems/day from the Story lane. This also settles the retirement question: because the backlog lane never withheld gems from the Story queue, retiring it releases nothing into that queue and cannot starve the 21:00 reel of quota. Verified no reel lane has been quota-blocked; last 24h ran 22/25 (17 Stories + 5 reels), and dropping the backlog lane's ~1/day frees a slot.
...
**FINAL LIVE STATE: everything off. `AUTO_EXPOSURE` and `EXPOSURE` unset, `AUTO_WB=false`, `WB_STRENGTH=0`, `HIGHLIGHT_STRENGTH=0.0`, `ORANGE_DESAT=1.0`, `SHARPEN_AMOUNT=0.0`.** Manual `-9` was tried and rejected by Boss. With the rolloff bug fixed and every post-processing pass disabled, the camera's own auto-exposure produces the best frame of the day — `mean 120.9, p1 32, p99 224, blown 0.04%`, natural colour, birds and foliage resolved, no fringing. **The correct answer for this camera was to stop processing it, not to process it better.** Every pass in the chain was written for a brooder under a tungsten heat lamp; none of it belongs on a daylight coop camera. Do not re-enable them here.

**⚠️ Operational trap — manual exposure LATCHES in the driver, and OpenCV lies about restoring it.** After setting `EXPOSURE=-9`, clearing the env vars did **not** return the camera to auto: "unset" means *don't touch*, so the camera stayed at the latched manual value and served near-black frames (`mean 12.7`). Explicitly setting `AUTO_EXPOSURE=auto` (3.0) and `0.75` both logged **`set_ok=True` and changed nothing** — the DirectShow driver accepted the call and ignored it. **The only thing that actually cleared it was a PnP device reset** (`Disable-PnpDevice` → `Enable-PnpDevice` on `USB\VID_32E6&PID_9221\...`), after which auto-exposure came back immediately (`mean 12.7 → 120.9`). If you ever set a manual exposure on this camera, budget a device reset to get back. Safe to do remotely **only** because this camera sits behind the VIA hub (`Hub_#0003`), not the root hub — a reset there would take the WiFi with it. Verified WiFi `Status: OK` in the same call.
...
**Reaction → published field note.** `scripts/diary-promote-on-reaction.py` + `com.farmguardian.diary-promote` (hourly) scan `#farm-2026` for the writer's diary posts, check whether **the Boss** reacted, and for each reacted day convert that day's `content/diary` entry into a published `content/field-notes/{iso-day}-{slug}.mdx`, then commit+push farm-2026 so it lands on `/field-notes`. Until now the reaction did nothing — the post literally promised "a reaction promotes it toward the public field notes" while no code consumed it, so the daily story only fed reel captions (invisible) and sat in Discord. farm-2026 CLAUDE.md already defines the intent (`content/diary` = raw source, field-notes = published); this is the sanctioned, Boss-gated bridge. Reuses `tools/discord_harvester` (token, Discord API, verified #farm-2026 id, git pattern); the title is lifted to frontmatter (leading H1 stripped so it isn't doubled), a cover is auto-picked from that day's carousel gems when present (the page guards `{cover && …}`, so none is fine), and the body is MDX-escaped. Commits only the one file (never `git add -A`, which would sweep the repo's other async committers) with a rebase-retry on push rejection; a `data/diary-promote-state.json` ledger + an existence check make it idempotent.

**Staleness alarm.** `scripts/diary-staleness-check.py` + `com.farmguardian.diary-staleness` (daily 09:00) import `daily_reel_runner`'s own eligibility logic (`_load_farm_context`, `_diary_date`, `_RESOLVED_RE`, `FARM_CONTEXT_MAX_AGE_DAYS`) so the alarm measures the **exact** context the captions receive — it can't drift from the real thing. It posts one Discord alarm (mentioning the Boss) when context is empty right now (RED), when the last usable entry ages out within 3 days (YELLOW), or when no entry has been written in >2 days — the 20:00 writer has likely died (YELLOW). Silent when healthy. This is the missing piece behind the whole outage: the diary went stale for weeks and nothing noticed until reel captions were visibly repeating one sentence.

**Two robustness bugs found and fixed during verification** (from dry-running against live data, not theory): the writer's slug drifts on `--force`, and its filename format changed to DD-Mon-YYYY in v2.51.11 — so the Discord post's stem no longer matched the file on disk (the first dry-run reported "diary file gone"). The promoter now resolves the on-disk entry by its **parsed date** (via the caption path's own `_diary_date`, which accepts both ISO and DD-Mon-YYYY), keying the ledger and the field-note name on the ISO day, so it's immune to both slug drift and the filename-format split.
...
**White balance (Boss: "fix the wb").** Once live, frames washed out with the cyan/purple/yellow posterisation from `docs/16-Apr-2026-heat-lamp-orange-cast-investigation.md`. That doc is about a heat lamp; **daylight is the opposite case.** Gray-world WB was over-correcting an already-balanced bright scene (only ~0.3% clipped) and manufacturing the rainbow. Set `USB_CAM_AUTO_WB=false` / `WB_STRENGTH=0` in GWTC's `start.bat` → clean, natural coop-run frames, balanced B/G/R ≈146/146/146 (was 170 with heavy posterisation). Persists across reboot (the scheduled task runs start.bat at logon). No exposure clamp needed. Documented the daylight-vs-heat-lamp distinction in the investigation doc so the existing "WB_STRENGTH=0 leaves a red cast" caveat (heat-lamp-only) doesn't get cited to revert this. GWTC's start.bat is now also checked into `deploy/gwtc/usb-cam-host/`.

**Screen — it's the hardware, and my earlier "fix" was wrong.** Windows enumerates the panel (BOE `NV116WHM-T16`), reports it Active, full power, brightness 90, driver healthy — yet it's black. That pattern is a **dead backlight / failed panel**, matching Boss's "maybe it's just fried". My 23-Jul claim that brightness was "pinned low and fixed" was incorrect: the brightness value takes (WMI reads 90) and the screen stays black, which rules brightness out. It does not matter operationally — GWTC runs headless fine. The `farmcam-screen-on` task is now pointless; leaving it pending Boss's call to remove.

**Built-in GWTC webcam:** still dead — `Hy-HD-Camera` reports `Present: False`, absent from the device bus. It does not work at all; the USB camera is what serves `usb-cam`.
...
**Fix 1 — give it something real to say.** New `_todays_observations()` summarizes the last 24h of already-VLM-enriched frames into caption material: which behaviours were actually observed (foraging, drinking, dust-bathing, preening), peak birds-in-frame, how much of the day was dim, whether a special bird appeared. ~3,300 enriched rows a day were being recorded and none of it reached the caption. One SQL query, no model calls, and it changes daily. The diary is demoted to "older background," retained only because it holds the sole source of bird *names* (the VLM's `individuals_visible_csv` is still generic `adult`/`chick`). The do-not-repeat instruction now demands a different **subject**, not just different phrasing.

**Fix 2 — enforce the brand rules instead of asking nicely.** Told "do not mention cameras… no hawk lines," the 4B local model emitted *"no one looking at the camera, no one chasing a hawk"* — two policy violations in one caption, which would have posted publicly. Naming a thing in order to negate it is still naming it, and small models are especially prone to it. So `BRAND_RULES` was rewritten as positive voice guidance that enumerates nothing, and the prohibitions moved into `brand_violations()` — a whole-word blocklist checked **in code** after generation, following the existing "defense in depth" precedent in `bird_photo_ingest.py`. A violating caption is regenerated (3 attempts, positive corrective nudge) and never patched: deleting "hawk" from a sentence about hawks leaves a sentence about nothing. If all attempts fail it posts the deterministic literal — dull but always safe. Blocklist covers production leakage (camera/footage/stream/AI), content policy (hawk/predator/attack/security), and the grown-flock rule (chick/baby).

**Also fixed:** a timestamp-comparison bug in the new query. `ts` is stored ISO-with-`T` (`2026-07-23T09:43:07+00:00`) while `datetime('now','-24 hour')` is space-separated; `"T"` sorts after `" "`, so every row sharing the cutoff's date compared greater and the "24h" window silently stretched to ~24-48h. Now bounded with a matching-format string, which is both correct and still index-friendly.

**Verified before commit** (read-only, nothing posted): all four lanes produce distinct captions grounded in the day's real behaviour, with zero brand violations — e.g. *"The flock's been busy foraging under the late afternoon sun, dust-bathing in patches of dry grass, and napping in the shade"*. Validator unit-checked, including that "chickadee" does not trip the `chick` rule. Degraded paths return empty cleanly (pipeline down, unreadable DB) and fall through to diary-only, then to the literal.
...
- **`_generate_reel_caption` unified on LM Studio.** The no-drafts case no longer short-circuits to the literal; it prompts the model like any other lane. All caption paths are text-only — no images are sent.
- Prompt restructured so the subject leads and the farm diary is explicitly demoted to background.

**Scope correction (worth recording, because the first pass at this got it wrong).** An intermediate version of this change attached sampled frames to the caption call to "ground" the timelapse lanes, on the assumption that all three fixed daily camera reels had been posting the literal. Checking the data rather than the assumption: `s7-cam` is **not** a `vlm_bypass` camera — 17,449 of 17,449 rows in the last 7 days carry a `caption_draft`, so the s7 lane always took the drafts path and its captions were never the literal. Only `house-yard` and `duo2` (both `vlm_bypass`, zero drafts) were affected, and those two are permanently aimed at the same yard, where a time-lapse looks the same every day. Boss's call: don't spend vision calls there. The frame-attachment code was therefore removed entirely rather than left behind a flag — with s7 excluded it had no lane that would ever use it.

**Net behaviour:** house-yard and duo2 captions are carried by the farm diary (the only input that varies day to day) plus their scene hint, which is appropriate for a fixed yard time-lapse and is still a large improvement on the frozen literal they posted for two weeks. s7 and the mixed lane continue to synthesize from per-frame `caption_draft` values. Fallback order is model → deterministic literal, so a caption always exists even with LM Studio down. Verified against live data before commit, read-only, nothing posted.
...
- **D8 (curation + pacing, s7-daily lane only):** the previously-inert `codex_reel_curator.curate()` now prunes redundant/weak frames from the s7-daily reel before stitching (falls back to the raw selection unchanged on any Codex failure — never breaks the reel). A one-frame "hook" duplicate holds the opening frame for an extra beat, scoped to a separate stitch-only list so it never double-counts in caption synthesis or the posted-state record.
- **D10 (camera-of-the-day rotation):** new `CAMERA_OF_THE_DAY_POOL` (mba-cam, usb-cam, dominator-cam, duo2 — gwtc excluded as a disabled camera, house-yard excluded as a lane that's never had a live plist) + `pick_camera_of_the_day()` (day-of-year modulo, deterministic) give the stacking evening timelapse lanes a shared rotation entry point: new `scripts/ig-camera-of-the-day-reel.py` + `deploy/ig-scheduled/com.farmguardian.ig-camera-of-the-day-reel.plist` (20:15, written but **not yet activated** — see followups). Existing per-camera lanes/plists are untouched and keep running alongside it for now; each lane's posted-state file is keyed by lane_id+date, so no double-post risk if both fire the same day.
- **D11 (sign-off rotation):** `build_caption`'s hardcoded `"📸 @markbarney121"` default is now a 3-item day-of-year rotation (same handle, varying lead emoji) via a sentinel that preserves the existing None/""-omits-the-line contract for callers who pass one explicitly.
- **B1 (nightly Instagram insights fetch):** new `tools/pipeline/ig_insights.py` — `ensure_insights_schema()` (self-contained, additive `ig_media_insights` table, run against production `guardian.db` and verified present), `fetch_media_insights()`/`fetch_follower_count()` (real Graph API v21 GETs), `run_nightly_fetch()` (per-media time series from `ig_posted_captions`). **Metric-name correction found via live testing:** the plan's assumed `plays` metric is rejected by the live v21 API (400) — `views` is current; normalized to `plays` at parse time so the DB column name is unaffected. Verified live against the real account: follower count 406, a real reel probe (`{"plays":5,"reach":5,...}`), a real carousel probe. New `scripts/ig-insights-fetch.py` + `deploy/ig-scheduled/com.farmguardian.ig-insights-fetch.plist` (23:30, written, not activated).
- **B2 (weekly Discord digest):** `build_weekly_digest()`/`post_weekly_digest()` — best/worst post by engagement, posts-by-surface, follower delta, posted to `#farm-2026`. New `scripts/ig-weekly-digest.py` + `deploy/ig-scheduled/com.farmguardian.ig-weekly-digest.plist` (Sunday 20:00, written, not activated). `ig_posted_captions` has zero rows as of this writing (no post has gone out since that ledger landed today), so the digest's non-empty branches haven't been exercised against real data yet — will self-verify on the first live nightly run.
- **E2 (local flock growth timelapse — NOT posted, NOT committed):** new `tools/pipeline/growth_timelapse.py`, run against production data. Bucketed s7-cam strong+sharp gems since 2026-04-16 (one/day, highest-`discord_reactions` representative per bucket) into an 83-frame, 83.2s, 1080×1920 MP4 at `~/bubba-workspace/scratch/growth-timelapse-s7-2026-07-16.mp4`, spanning 2026-04-16→2026-07-16. **Explicitly a manual review tool per the plan** ("stitch locally and review before any publish") — no LaunchAgent, no auto-publish wiring. Flagged during review: 9 of the 83 frames are native landscape (6 pre-portrait-switch, 3 later stray landscape captures on Apr 25/Jul 4/Jul 5) and get soft-upscaled by `reel_stitcher`'s existing mixed-resolution handling — worth an eyeball, not urgent.

**Correction to the original diagnosis (Part D's "carousel starved to zero" claim):** verified today against `ig-daily-carousel`'s own logs and `image_archive` — the carousel lane was **not** actually starved to zero; it successfully posted on 07-12, 07-13, and 07-14 (confirmed via successful "posted carousel of N gems" log lines and live Graph API media listing). The real bug: reels and carousels shared one `ig_permalink` column, so when a reel later reused a gem that a carousel had already successfully posted, the reel's write-back **silently overwrote** the carousel's permalink record — corrupting the audit trail and letting the reel selector re-post the same source photo a second time on a different surface. Today's earlier `reel_permalink`/`used_in_reel_at` column split (v2.47.0, D1) is the correct fix for this — for the corrected reason, not the originally-diagnosed one. The 18:00→12:30 schedule move remains a reasonable structural improvement independent of this correction.

**Why:** measurement (B1/B2) closes the open-loop problem the whole Part D effort was otherwise flying blind on; D8/D10/D11 are the remaining "reels well put together" items; E2 proves the flock-level growth-timelapse capability the plan promised is real, working, and ready for Boss's review.

**How:** all new LaunchAgent plists are written but **not activated** — see the plan doc's execution guardrails (no live-posting behavior flips without explicit per-flip approval). Exact `launchctl bootstrap` commands are in the plan doc's status section.
...
**Why:** DB-verified starvation — every completed day 09→14-Jul left zero carousel-eligible gems because the 18:00 reel consumed the pool; timelapse reels posted identical captions daily; nothing prevented caption/tag repetition because posted captions were never stored.

**How:** `store.py` (schema: 2 late columns, 1 new table — migration applied 16-Jul after `backups/guardian-pre-v2.47-20260716.db`), `ig_poster.py` (`_write_reel_usage`, `record_posted_caption`, `recent_posted_captions`, `recent_tags_used` + three publish-path hooks), `daily_reel_runner.py`, `codex_reel_curator.py`, `orchestrator.py`, `ig_selection.py` (comment). NOT changed: photo lane stays disabled (per Boss — carousel-only until insights data exists); reel cadence untouched.
...
**Why:** v2.45.0 broke Discord posting: `#farm-2026` went silent for a full day (last post 12-Jul 19:57; 0 posts across the 420 frames scored on the new scale). Root cause was a **miscalibration, not the daemon or the preset** (both verified live: the 4b VLM emits `expression_score`/`detail_score` on 100% of frames, and `_compute_overall_score` matched stored `overall_score` on 420/420 frames — the new code IS running). The dominance mapping was the bug: s7-cam (the only real gem source) is a wide-angle phone cam, so even a bird posing right at the lens fills only ~30–40% of the frame, scoring ~9–12 of 30 for dominance. That capped every genuine gem at ~65–72 — under the 80 gate. The median historically-posted gem was *mathematically* unable to reach 80 (it would need expression+detail ≥ 56 of a max 55). v2.45.0's own tests passed because they used **synthetic hand-set scores (85, 55), never live VLM output**. A bird filling half a wide frame IS the dominant subject, so 50% now earns full dominance. The ping ≥95 was likewise unreachable (real component ceiling ~92, since the 4b model never emits the full expression 30 / detail 25), so it never fired; ≥90 restores Boss's bird-selfie @-mention.

**How:** `orchestrator.py` — dominance divisor 100→50 in `_compute_overall_score` (docstring updated with the wide-angle-cam rationale), ping threshold 95→90 in the caption block, header bumped. **Verified against REAL VLM output, not synthetic:** replaying the new formula over all 420 live new-prompt frames, the one genuine strong gem (expr 25, detail 22, sharp, `largest`=35) rises **72 → 83** and clears the gate, while **0 of 33 `decent`** (max 64) and **0 of 386 `skip`** (max 37) frames breach 80 — the multi-axis design and the floor-pecking caps (30/40) still hold the line, so no re-flood. Both `test_gem_poster_gate` and `test_floor_pecking_calibration` still pass (0 failures). Thin live data (the new prompt only went live 13-Jul 11:25, so just one strong frame so far) — monitoring the first live posts after restart to confirm cadence and tighten if needed. **Requires a pipeline daemon restart** (`launchctl kickstart -k gui/$(id -u)/com.farmguardian.pipeline`).

### v2.45.0 — gem score reworked to a 0–100 weighted-component scale, gate raised to 80+ (Claude Opus 4.8, Bubba) — 12-Jul-2026

**What:** The Discord gem score moved from a holistic 0–10 VLM number to a **0–100 score computed in code from four weighted components**: frame dominance (0–30, derived from `largest_subject_pct`), expression/absurdity (0–30, VLM `expression_score`), notable detail (0–25, VLM `detail_score`), and technical quality (0–15, derived from `image_quality`+`lighting`). New `orchestrator._compute_overall_score()` runs right after the VLM returns and overwrites `overall_score`; the VLM's own guess is discarded. `schema.json` gains two required fields (`expression_score` 0–30, `detail_score` 0–25) and `overall_score` max is now 100. The post gate `_MIN_OVERALL_SCORE` went 7 → **80** — only 80+ gems reach `#farm-2026`. Floor-pecking calibration caps rescaled (3→30, 4→40; skip-line 4→40). Caption suffix `⭐ N/10` → `⭐ N/100`; the Boss "BIRD SELFIE" ping now fires at score ≥ 95 (the "99%-er") instead of exactly 10. The live LM Studio `Birds.preset.json` (systemPrompt + jsonSchema) was regenerated from the repo source-of-record, which also propagated the previously-staged mixed-age/close-and-looking prompt edits that had never been pushed to the live preset.

**Why:** Boss: the 0–10 scale "is not granular enough… needs to be 0–100 and I really only want to see the ones that are 80 plus." A single 0–100 number from a 4b VLM does not actually buy granularity — it bunches on round numbers. Decomposing into four small axes summed in code produces a real spread and lets code own the weighting. Two axes reuse fields the VLM already emits (`largest_subject_pct`, `image_quality`/`lighting`) so they can't contradict the rest of the metadata (DRY) and the VLM only has to rate two small concrete ranges it can handle. Boss's canonical 99%-er — a frame-filling bird with a ridiculous expression and a visible monster claw — scores ~97 and trips the ping.

**How:** `schema.json` (2 new required fields, `overall_score` max→100), `orchestrator.py` (`_compute_overall_score` + call site before floor-pecking calibration, cap rescale, caption `/100` + ≥95 ping), `gem_poster.py` (`_MIN_OVERALL_SCORE=80` + stale 0–10 prose in comments/docstring corrected), `prompt.md` (component rubric, rescaled anchors + calibration examples, close-and-looking cap reframed onto `share_worth`+components). `share_worth` tier strings are unchanged, so `store`/`ig_selection`/retention/DB are untouched; `overall_score` is not a DB column (lives in `vlm_json`), so no migration. **Verified:** both `test_gem_poster_gate` and `test_floor_pecking_calibration` at 0 failures in the pipeline venv; a component dry-run and a full compute→calibration→gate round trip confirm the 99%-er→97 posts, a solid gem→83 posts, a calm portrait→60 is held back, and a floor-peck scatter→demoted to 30/skip. Preset backed up to `Birds.preset.json.bak-<ts>`; repo files backed up to `.score100-backup-<ts>/`. **Requires a guardian daemon restart to pick up the new preset.**
...
**What:** Three changes in response to Boss's order ("Tighten motion detection on BOTH Reolink cameras — still way too sensitive. And make sure the NEW Reolink camera is also cutting a timelapse reel like the others."): (A) added a separate, larger Guardian-side debounce for the non-YOLO camera-motion Discord alerts; (B) dialed DOWN onboard motion-detection (MD) sensitivity on both Reolinks (the real common culprit, applied on the cameras themselves); (C) wired duo2 into the daily time-lapse reel system like the other cams and proved a reel builds end-to-end.

**Why:** The "still too sensitive" complaint had two layers. The dominant one is **camera-side onboard MD sensitivity** — duo2 was at 41/50 (very high) and house-yard at 25/50; the camera's own motion events are what drive Guardian's `get_motion_state` → burst → motion alert. Prior config-only tweaks missed this because they tuned Guardian, not the camera. The secondary layer is the Guardian motion-alert path (`_motion_watch_loop` → `send_motion_alert`), which posted a pure-motion Discord embed on every camera MD transition with only the 90s predator cooldown — spammy. duo2 produces ZERO Guardian alerts (raw RTSP, `detection_enabled:false`), so its only motion alerting was the camera's onboard layer — confirming camera-side is the right knob for it.

**How:**
- **(B) Camera-side MD sensitivity (the Boss's literal order), via `reolink-aio`, sensitivity ONLY:** house-yard `md_sensitivity` **25 → 18**; duo2 `md_sensitivity` **41 → 28** (range 1–50; moderate notch down, not minimum). Read-back confirmed both took. **AI sensitivity left untouched** (dog_cat/people/vehicle = 60/60/60 on both) — that is what catches real hawks/people, so lowering it is a false-negative risk reserved for a Boss decision. Spotlight / floodlight / siren / auto-track all untouched. Live IPs are house-yard `192.168.0.89`, duo2 `192.168.0.156` (the HARDWARE_INVENTORY `.88`/`.14` values are stale — both failed to connect).
- **(A) Guardian-side debounce (`alerts.py` + `config.json`):** new `motion_alert.cooldown_seconds` (set to **600s**) drives a dedicated `_motion_cooldown_seconds` for `send_motion_alert`, separate from `detection.alert_cooldown_seconds` (90s, predator alerts — unchanged). Pure-upside: throttles the spammy non-YOLO motion posts ~6.7× harder without touching predator alerts. NOTE: this does not make motion alerts YOLO-gated — they still emit "motion sensor triggered" posts, just rarely. Gating them behind YOLO would make them redundant with the existing YOLO `send_alert` path (their documented purpose is to surface what YOLO misses), so a hard debounce is the right lever. `motion_alert.enabled` left `true`; flip to `false` for zero non-YOLO alerts (Boss decision).
- **(C) duo2 time-lapse reel lane:** duo2 was ALREADY captured by the pipeline (8,559 raw rows/24h in `image_archive`, frames in `data/archive/2026-06/duo2/raw/`) — capture was not the blocker. Added `select_duo2_timelapse_gems` wrapper (`ig_selection.py`), `DUO2_TIMELAPSE_LANE` (`daily_reel_runner.py`, `landscape_mode=True`), thin script `scripts/ig-duo2-timelapse-reel.py`, and LaunchAgent `com.farmguardian.ig-duo2-timelapse-reel.plist` (daily 21:20, loaded). Mirrors the dominator-cam lane.
- **Verified:** Built a real reel via the production `select_duo2_timelapse_gems` → `stitch_gems_to_reel` chain (NOT the auto-publishing `main()`): 60 frames → `/tmp/duo2-timelapse-PROOF.mp4`, 20.9 MB, 1920×1080 H.264, 51s. Extracted a mid frame — duo2's wide panoramic is **letterboxed undistorted** inside the 16:9 frame (fisheye curves natural, "Duo2"/Reolink overlays intact). Guardian restarted via `launchctl kickstart -k`; one PID (62340), all 6 cameras `online:true, capturing:true`, log shows "Motion watcher started for cameras: house-yard" + "Motion-alert Discord posts ENABLED". `motion_alert.cooldown_seconds` confirmed loaded (predator 90s / motion 600s).
- **Backups:** `config.json.bak.pre-reolink-tighten.20260622-204814`, `alerts.py.backup.20260622-204814`, `tools/pipeline/daily_reel_runner.py.bak.20260622-204814`, `tools/pipeline/ig_selection.py.bak.20260622-204814`. Camera rollback is one `set_md_sensitivity` call each (house-yard→25, duo2→41).
- **Not touched (flagged for Boss):** YOLO `bird` class-confidence threshold (0.45) — a SEPARATE potential spam source on the YOLO `send_alert` path (not the motion path); left alone deliberately since it is also how a hawk/crow gets caught. duo2 YOLO detection still off. The stale `.14` IP in duo2's `tools/pipeline/config.json` `context` string (cosmetic, vlm_bypass — never read) left as-is to avoid touching the live pipeline config.

### ops(diagnosis) — 16-Jun-2026: investigated "two farm pipelines have output problems" — NO bug, NO code/config change (Claude Opus 4.8 (1M context), Bubba sub-agent)

**What:** Triaged two complaints — "birds posing on the S7 but nothing hits Discord" and "VLM calls sharp S7 frames blurred/bird_count 0/skip." Outcome: both are explained without a code change. **No files were modified** beyond this CHANGELOG and the provenance memory log.

**Findings (evidence in pipeline log /tmp/pipeline.err.log + live A/B):**
- **VLM endpoint confirmed = LOCAL LM Studio**, not Gemini. `tools/pipeline/config.json:5` `"lm_studio_base":"http://localhost:1234"`, `:6` `"vlm_model_id":"qwen/qwen3.5-9b"`; used at `orchestrator.py:422-423` → `vlm_enricher.py` POSTs `{lm_base}/v1/chat/completions`. The per-frame classifier and the reel-caption synthesizer (`daily_reel_runner.py:604`) are both LM Studio. No Gemini in the classification/caption path. (Corrects a prior session's "captions use Gemini" claim.)
- **S7 "posing birds" complaint targets the wrong pipeline.** s7-cam has `detection_enabled:false` in `config.json` **by design** — s7 watches the resident flock, and YOLO's COCO `bird` class can't separate a hawk from a hen, so enabling the guard-dog on s7 "bird-only" would fire `Predator Alert: Bird` on the farm's own chickens every cooldown. The correct mechanism for "cute posing bird → Discord" is the GEM pipeline (`orchestrator → should_post → post_gem`), which already runs on s7 every 7s.
- **The gem pipeline IS working and posting.** Log shows s7 strong gems `"posted_to_discord": true` at 09:13:15 and 10:53:44 on 16-Jun. The VLM correctly promotes genuine gems and correctly skips non-gems.
- **"Sharp frame mislabeled" is false.** A/B via the pipeline's own `_downscale_for_vlm` + `enrich`: this morning's known gem rated `strong`/`sharp`/score 8 at 768px; current live frames (bird rumps, backs-to-camera, out-of-focus close-ups) rated `skip` at **both 768px and 1536px** — resolution does not flip the verdict, and the skips match the rubric (a sharp photo of a backside is still a skip). `vlm_input_long_edge_px:768` has been constant since 06-06 and produces gems fine — it is **not** the cause. Did not change it (would add latency and destabilize a working, Boss-trusted s7 gem gate).
- **Likely real reason Boss "sees nothing":** the "Farm Guardian" webhook (gems + predator alerts) posts to channel **1482466978806497522**; the Boss is complaining in **#meet-the-lobsters 1471632572953006337** — a different channel in the same guild. Possible channel mismatch — flagged for Boss/Horst, not changed unilaterally.

**Secondary (noted, not fixed):** gwtc (rtsp 192.168.0.69:8554) and house-yard ONVIF are offline, flooding logs. house-yard is the *only* `detection_enabled:true` cam, so the guard-dog currently has no online detection camera — separate issue from the above.

### v2.42.0 — feat: golden-window time-lapse reels for usb-cam + dominator-cam (two daily activity windows, dynamic sunrise, denser in-window sampling, off-peak capture trim) (Claude Opus 4.8 (Bubba sub-agent))

**What:** Reworked the usb-cam (coop-run) and dominator-cam time-lapse REEL lanes so they cover only the two daily "golden" activity windows the Boss cares about — **morning sunrise→09:00** and **evening 19:30→20:30** (local America/New_York) — sampled denser within those windows, and trimmed the wasteful all-day raw CAPTURE on these two cams to a slow off-peak heartbeat. Boss-approved 2026-06-14 in #meet-the-lobsters ("anything during the day is just junk"). Scope is strictly these two `type=fixed` secondary cams; s7-cam, mba-cam, gwtc, and the house-yard PTZ (.88, the only Stage-0 predator-motion source) are untouched.

**Why:** The old reels spread ~60 frames across the whole 06:00–20:00 daylight band, so consecutive frames were ~14 min apart and most of the reel was empty midday coop. usb-cam was also writing ~40k raw JPEGs/day (every ~2s, 24/7) and dominator ~8.5k/day — captured around the clock including all night — almost all of it never used (blast-radius check below). Birds are active at dawn and dusk; the rest is junk.

**How:**
- New shared module `tools/pipeline/golden_windows.py` — dependency-light (stdlib `math`+`zoneinfo`; `astral` is not in the venv) NOAA sunrise/sunset calc + minute-granular window-membership primitive. Single source of truth used by BOTH selection and capture so they agree on "in a golden window." Sunrise computed dynamically for the farm (41.7558 N, 71.9789 W) so the morning window tracks the season — verified 05:13 ET (2026-06-14) and 07:12 ET (winter solstice) against reference. 24 unit tests in `test_golden_windows.py`.
- **Lever 1 (selection — `tools/pipeline/ig_selection.py`):** for cameras listed in the new `instagram.scheduled.timelapse_golden_windows.cameras`, the single daylight window is REPLACED by the two configured windows (per-lane via `per_camera` overrides), with denser **seconds-granular bucketing** (`sample_bucket_seconds`, default 30s) instead of 5-min buckets. Chronological order preserved (time-lapse value). Extended `_is_local_hour_in_window` to delegate to the shared minute-granular `minute_in_window` (DRY — hour and minute paths share one primitive). s7/mba/gwtc keep their existing behavior (not in the cameras list).
- **Lever 2 (capture — `tools/pipeline/orchestrator.py`):** `_run_raw_camera_thread` now recomputes cadence every iteration — **thick `cycle_seconds` inside the windows, sparse `offpeak_cycle_seconds` (180s heartbeat) outside** — for cams that opt in via the same golden config and set `offpeak_cycle_seconds`. Not a full stop: the cam stays provably alive and an incidental midday bird isn't 100% lost. Mirrors the house-yard `night_snapshot_interval` precedent (per-camera interval values, shared time-of-day predicate). Cams without `offpeak_cycle_seconds` are unchanged.
- **Config (`tools/pipeline/config.json`):** added the `timelapse_golden_windows` block (windows, lat/long, `sample_bucket_seconds`, `max_frames` 90, `min_frames` 6), added `offpeak_cycle_seconds: 180` to usb-cam and dominator-cam (their `cycle_seconds` left untouched). usb-cam is KEPT in the legacy `timelapse_reel_daylight_only_cameras` list as its off-state fallback (golden takes precedence in code when enabled; daylight 06:00–20:00 when disabled — so `enabled` is a true clean toggle, not an all-hours regression). All new code reads `cfg.get(key, default=current-behavior)` and the whole feature is gated by `timelapse_golden_windows.enabled`, so it is a no-op until flipped on.
- **Blast-radius check:** the ONLY consumers of these two cams' raw frames are their two time-lapse reel lanes. Carousel/story/daily/weekly selectors all gate on `discord_reactions >= 1` (and weekly on `share_worth='strong'`), which raw frames never have, and both cams are in `gem_poster._GEM_POST_DISABLED_CAMERAS`. Trimming off-peak raw capture starves nothing. Predator detection is unaffected: both cams are `detection_enabled=false`, `type=fixed`, feed no hardware-motion events; .88 (the Stage-0 source) is not touched.
- **Verified:** production selectors return exactly 90 frames, ALL inside the golden windows, chronological, morning starting at the dynamically-computed 05:13 sunrise; both lanes dry-run-stitched to valid 1920×1080 H.264 reels (76.6s); sampled source frames are bird-bearing in the active morning window (post-sunset evening tail thins as birds roost — honest caveat); Lever 2 cadence decision flips thick/sparse correctly at every boundary; existing `test_ig_selection_timelapse` still passes.

### v2.41.3 — fix: mba-cam brooder time-lapse was never capturing raw frames (enabled flag was off) (Claude Opus 4.8 (Bubba sub-agent))

**What:** The MacBook-Air brooder cam (`mba-cam`) reel lane had run every night since 2026-06-07 but never built — the selector logged `no raw frames for mba-cam in last 24h` and bailed. Root cause: `mba-cam` was `"enabled": false` in `tools/pipeline/config.json`, so the orchestrator never spawned a raw-capture thread for it. The downstream reel lane (`MBA_CAM_TIMELAPSE_LANE`, scheduled 20:30) was wired up, but the upstream CAPTURE was never turned on. Last raw row before this fix was `2026-05-12`.

**Why:** The camera's config `context` note claimed the on/off switch was "loading/unloading the MBA LaunchAgent; no config change needed." That is wrong: the MBA LaunchAgent only *serves* the `:8089/photo.jpg` snapshot endpoint. The orchestrator gates raw capture on the per-camera `enabled` flag (`_run_raw_camera_thread` launch loop only starts threads where `enabled and vlm_bypass`), independent of whether the endpoint is reachable. With `enabled:false` the brooder snapshots were live but nothing pulled or stored them.

**How:**
- Flipped `mba-cam.enabled` `false → true` in `tools/pipeline/config.json` (timestamped backup `config.json.bak.pre-mba-enable.20260614-144433` taken first; diff verified to be the single flag + the corrected note, nothing else). Mirrors the already-working `dominator-cam`, which is `enabled:true` against an equally-opportunistic endpoint and pulls ~15k raw rows/24h.
- Corrected the misleading `context` note to state the real on/off switch is the `enabled` flag (the bug's root cause; left it accurate to prevent recurrence). `vlm_bypass` means this string is not used for VLM, so editing it has no runtime effect.
- Restarted `com.farmguardian.pipeline` once via `launchctl kickstart -k` (KeepAlive service; not a kill loop). New PID 27789.
- **Verified:** raw-capture thread started (`mba-cam: raw-capture thread started (cadence 10s, vlm_bypass=true)`); frames now land every 10s with `status:ok` (1280×720 JPEGs into `archive/2026-06/mba-cam/raw/`); the stale 8,553 May frames were pruned by the rolling 48h sweep (2.7 GB freed); usb-cam/dominator/house-yard kept writing through the restart. Selector message flipped from `no raw frames for mba-cam` to `only N bucketed frames … (need >=6)` — i.e. it now finds them, just needs accumulation. Ran the **production** `select_timelapse_gems` + `stitch_gems_to_reel` end-to-end on the live frames (thresholds lowered only to fit the first minutes of capture) → produced a valid 1920×1080 H.264 MP4, proving the full capture→select→stitch path works for `mba-cam`. Tonight's 20:30 run (24h window, ≥6 frames) will have ~5+ hours of frames (~60 buckets) and auto-post the brooder reel.
...
- **Config repoint (both files, per the two-config rule):** `config.json` `usb-cam.http_base_url` and `tools/pipeline/config.json` `usb-cam.ip_webcam_base` → `http://192.168.0.194:8090`. Both LaunchAgents reloaded (`com.farmguardian.guardian` + `com.farmguardian.pipeline`); `/api/cameras` confirms `dominator-cam` and `usb-cam` both `online/capturing/is_live = true` (also verified live through the public Cloudflare tunnel at `guardian.markbarney.net`). The live `config.json` + `tools/pipeline/config.json` were deliberately NOT staged in this commit. **⚠️ Correction to a long-standing CLAUDE.md claim:** these two files are NOT gitignored — `git check-ignore` returns nothing and `git ls-files` shows both as TRACKED, and `config.json` currently holds live secrets in-tree (`alerts.discord_webhook_url`, the Reolink RTSP `password`). A future `git commit -am` WOULD commit those secrets. Pre-existing condition (secrets are already in history as of `54d3ec8`); flagged here for a proper follow-up (untrack + scrub or move secrets to env).
...
- **Durable fix DONE:** added a DHCP Address Reservation on the TP-Link Archer AX55 (Advanced → Network → DHCP Server → Address Reservation) binding GWTC's MAC `F0:35:75:81:2C:45` → `192.168.0.69` (Boss-authorized; the router lease was only 120 min, which is why the IP wandered). GWTC now holds `.69` across reboots. Re-add the reservation if the router is ever factory-reset.
...
**What changed:**
- **Durable model load** (`vlm_enricher.py::ensure_model_loaded`, called once at `orchestrator.py::run_daemon` startup): the pipeline now loads `qwen/qwen3.5-9b` itself at `context_length=16384, parallel=1, flash_attention=true` per `docs/13-Apr-2026-lm-studio-reference.md` (Rule 2: cap at 8k–16k; parallel-1 avoids the KV-slot split that contributed to the 2026-04-13 incident). Previously the pipeline never loaded the model and `vlm_load_context_length` was dead config — LM Studio JIT-loaded it at a cramped ~7115/slot. The new helper checks what's loaded first (never stacks instances), reloads only if the loaded context is too small, and is **non-fatal** — if LM Studio is down at startup the daemon logs and continues, and the existing per-cycle read-only skip handles a still-unloaded model. This is the single controlled exception to the otherwise read-only per-cycle LM Studio coordination. `vlm_load_context_length` bumped 8192 → 16384 in `tools/pipeline/config.json`. Verified by unloading the model and restarting the daemon: it reloaded at 16384 on its own. Measured per-frame split (LM Studio server log): prompt eval ~2 s cached / ~5 s cold, generation ~6 s / ~200 output tokens at ~34 tok/s (memory-bandwidth bound for a 9B model on the M4 Pro; `flash_attention` did not move it). Further latency cuts require fewer output tokens or a smaller model.
